...
`infra/` holds the adapters. `repository/` has the SQLAlchemy `<Entity>Model`, the mappers, and the `Sql<Entity>Repository` that implements the port. `http/` has the FastAPI router and Pydantic schemas. The router is the composition root. It wires the concrete adapters into the service.
